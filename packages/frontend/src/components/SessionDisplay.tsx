import { LuArrowRight, LuClock, LuDelete, LuDot, LuTrash } from "react-icons/lu";
import type { QuizSessionPublic, QuizSessionSecure } from "shared/types";

type SessionDisplayProps = {
	session: QuizSessionSecure;
	...
	return (
		<div className="flex gap-4 items-center py-1 px-2 rounded-sm  transition-colors hover:bg-surface-2">
			...
			<div className="ml-auto flex gap-1">
				<button onClick={actions?.onDelete} className="button flex items-center p-2 rounded-md gap-2">
					<LuTrash />
				</button>

				<button onClick={actions?.onSelect} className="button flex items-center p-2 rounded-md gap-2">
					<LuArrowRight />
				</button>
			</div>
		</div>
