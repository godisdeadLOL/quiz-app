...
async def get_many_sessions(
    filters: Annotated[SessionGetManyParams, Query()],
    db_context: DatabaseContext = Depends(get_db_context),
):
...
@router.put("/{session_id}/answers/{answer_index}", response_model=SessionDetailed)
...
@router.post("/finish", response_model=SessionDetailed)
